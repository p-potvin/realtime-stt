...
## 2026-05-19 - Avoid redundant tensor peak recalculation to prevent blocking sync
**Learning:** In audio processing loops, recalculating `torch.max(torch.abs(tensor)).item()` on a tensor that is a direct copy of a numpy array where the peak was already calculated is a performance anti-pattern. Not only does it needlessly recalculate the maximum (an O(N) operation), but `.item()` forces a blocking device-to-host CPU synchronization if the tensor resides on a GPU, drastically degrading latency in a hot real-time audio loop.
**Action:** Always reuse scalar properties (like peak amplitude) that have already been calculated on the CPU (e.g., using `np.max(np.abs(...))`) before transferring the data to a tensor, instead of lazily recalculating them on the tensor object.
