...
**Learning:** When downmixing multi-channel (e.g. stereo) NumPy arrays to mono in a high-frequency loop (`stt_engine/audio_capture.py`), using `data.sum(axis=1) / data.shape[1]` is measurably faster (~35%) than relying on `data.mean(axis=1)`. `mean()` internally has additional overhead and function dispatch. Using `sum()` with dynamic division preserves generic channel support while bypassing this overhead.
**Action:** When averaging multi-dimensional NumPy arrays across an axis on a hot path, prefer `.sum() / shape[axis]` over `.mean()`.
